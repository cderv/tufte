...
```{r fig-margin, fig.margin = TRUE, fig.align = 'center', fig.cap = "MPG vs horsepower, colored by transmission.", fig.width=3.5, fig.height=3.5, cache=TRUE, message=FALSE}
library(ggplot2)
mtcars2 <- mtcars
mtcars2$am <- factor(
...
For PDF output, the vertical offset of margin figures can be set globally via the `margin_fig_pos` argument of `tufte_handout()` and `tufte_book()` (a LaTeX length such as `"0cm"` or `"-5pt"`). It can also be set per-chunk with the `margin_fig_pos` chunk option, which lets a single chunk override the global default without interfering with regular figures' `fig.pos`.
...
References display as margin notes in HTML output when `link-citations: yes` is set in the YAML metadata. For example, we can cite R here [@R-base]. Otherwise, references appear at the end of the document.

Entries cited only via the YAML `nocite` field (above we use `'@*'` to include every bibliography entry) are listed at the bottom of the document, even when in-text citations are pulled into the margin.

For PDF output, citations via `citation_package: natbib` also route through margin sidenotes; `\citet` (used by `@key`) keeps its textual "Author (Year)" form inline while adding the full reference to the margin.

# Bookdown extensions

The output formats `tufte_handout2()`, `tufte_book2()`, and `tufte_html2()` wrap the corresponding **bookdown** output formats and add support for [text references](https://bookdown.org/yihui/bookdown/markdown-extensions-by-bookdown.html#text-references) `(ref:label)` in figure and table captions, and [cross-references](https://bookdown.org/yihui/bookdown/cross-references.html) (e.g. `\@ref(fig:label)`). Requires the **bookdown** package.

```yaml
output:
  tufte::tufte_html2: default
